...
public final class ColorFactory {
    ...
    private static final String[] COLORS = new String[]{
            RED, LIGHTBLUE, AMBER, ORANGE, YELLOW,
            LIME, BLUE, INDIGO, LIGHTGREEN, GREY, DEEPPURPLE, TEAL, CYAN
    };

    public static int[] onRandomBuild() {
        int random = (int) (Math.random() * COLORS.length);
        int bgColor = Color.parseColor(SHARP + BG_COLOR_ALPHA + COLORS[random]);
        int bdColor = Color.parseColor(SHARP + BD_COLOR_ALPHA + COLORS[random]);
        int tColor = SHARP666666;
        return new int[]{
                bgColor, bdColor, tColor
        };
    }

    public static int[] onPureBuild(PureColor type) {
        String color = type == PureColor.CYAN ? CYAN : TEAL;
        int bgColor = Color.parseColor(SHARP + BG_COLOR_ALPHA + color);
        int bdColor = Color.parseColor(SHARP + BD_COLOR_ALPHA + color);
